...
/// `emit_shared_artifact_gate` says so on every release build here, in a notice
/// that names the object, the reason and the archive to use instead. That
/// notice is recorded in the build script's captured output and in
/// `DROP-IN-CDYLIB-NOTICE.txt` in `OUT_DIR` rather than emitted as a
/// `cargo:warning`, because a diagnostic that fires on every build of a
/// supported configuration is one nobody can clear and 0.9.2 `A1` asks both
/// configurations to build cleanly.
/// It announces rather than refuses because `crate-type` belongs to
